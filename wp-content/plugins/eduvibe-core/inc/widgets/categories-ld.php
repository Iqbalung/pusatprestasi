...
/*
 * EduVibe LearnDash Categories Widgets
 */ 
if( ! function_exists( 'eduvibe_ld_register_categories_widget' ) ) :
	function eduvibe_ld_register_categories_widget() {
	    register_widget( 'EduVibeCore_LD_Categories_Widget' );
	}
endif;

add_action( 'widgets_init', 'eduvibe_ld_register_categories_widget' );

if( ! class_exists( 'EduVibeCore_LD_Categories_Widget' ) ) :
	class EduVibeCore_LD_Categories_Widget extends WP_Widget {

		function __construct() {
			parent::__construct(
				'EduVibeCore_LD_Categories_Widget',
				__( 'EduVibe Categories( LearnDash )',  'eduvibe-core' ),
				array( 'description' => __( 'EduVibe LearnDash Course Categories',  'eduvibe-core' ), )
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			$course_categories = get_terms( 'ld_course_category', $args );
            if ( is_array( $course_categories ) ) : 
                echo '<div class="widget-categories eduvibe-course-category-widget">';
                    foreach ( $course_categories as $course_cat ) :
                        $term_link    = get_term_link( $course_cat, 'ld_course_category' );
