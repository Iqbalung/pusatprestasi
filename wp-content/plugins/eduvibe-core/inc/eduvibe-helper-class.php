...
class Helper {
    ...
    public static function retrieve_posts( $post_type, $select_post = false ) {
        ...
        $select_one = __( 'Select a Post', 'eduvibe-core' );
        $no_post    = __( 'No Post Found. Please, Create one First', 'eduvibe-core' );
        if ( 'wpcf7_contact_form' === $post_type ) :
            $select_one = __( 'Select a Form', 'eduvibe-core' );
            $no_post    = __( 'No Form Found. Please, Create one First', 'eduvibe-core' );
        elseif ( LP_COURSE_CPT === $post_type ) :
            $select_one = __( 'Select a Course', 'eduvibe-core' );
            $no_post    = __( 'No Course Found. Please, Create one First', 'eduvibe-core' );
        elseif ( 'simple_event' === $post_type ) :
            $select_one = __( 'Select an Event', 'eduvibe-core' );
            $no_post    = __( 'No Event Found. Please, Create one First', 'eduvibe-core' );
        endif;
    ...
    public static function retrieve_mailchimp_items() {
        ...
        if ( ! empty( $lists ) && ! empty( $lists->lists ) ) :
            $lists_name = array( '' => __( 'Select a mailChimp Form', 'eduvibe-core' ) );
    ...
    public static function masking_shapes() {
        $path         = EDUVIBE_ASSETS_URL . 'images/masking-shapes/';
        $shape_prefix = 'shape';
        $format       = '.svg';
        $shapes       = [];
        for ( $i = 1; $i <= 9; $i++ ) :
